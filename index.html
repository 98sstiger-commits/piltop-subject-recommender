...
<head>
  ...
  <style>
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .tut-left{display:flex;align-items:center;gap:8px;font-size:14px;font-weight:600;color:var(--text)}
    ...
    .fp-card{max-width:660px;width:100%;background:#fff;border-radius:18px;padding:28px 24px 24px;box-shadow:0 8px 40px rgba(0,0,0,.18)}
    ...
    .fp-err{margin-top:10px;padding:10px 13px;background:#fef2f2;border:1px solid #fecaca;border-radius:8px;color:#dc2626;font-size:13px;line-height:1.6}
    ...
    .rp-sn{font-family:var(--fd);font-size:28px;font-weight:900;color:var(--cream);line-height:1}
    .rp-sn.red{color:#e74c3c}
    .rp-sn.amb{color:var(--gold)}
    .rp-sl{font-size:11px;color:rgba(237,229,204,.38);margin-top:4px}
    .rp-wrap{padding:0 22px 56px;margin-top:-44px;position:relative;z-index:10}
    .rp-card{max-width:940px;margin:0 auto;background:#fff;border-radius:18px;padding:24px;box-shadow:0 8px 40px rgba(0,0,0,.18)}
    .rp-top2{display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:10px}
    .chips{display:flex;gap:5px;flex-wrap:wrap}
    .chip{padding:4px 11px;background:#f5f0e6;border:1px solid #e0d8c0;color:#7a6228;border-radius:20px;font-size:11px;font-weight:600}
    .rp-btns{display:flex;gap:7px;flex-wrap:wrap}
    .rb-lnk{padding:7px 13px;background:var(--bg2);border:none;border-radius:8px;color:var(--cream);font-size:12px;font-weight:600;cursor:pointer;font-family:var(--fb);white-space:nowrap}
    .rb-lnk:hover{background:var(--bg3)}
    .rb-pdf{padding:7px 13px;background:var(--gold);border:none;border-radius:8px;color:#fff;font-size:12px;font-weight:700;cursor:pointer;font-family:var(--fb);white-space:nowrap}
    ...
    .strat-l{font-size:10px;font-weight:800;color:#8a6e1a;letter-spacing:.12em;text-transform:uppercase;margin-bottom:5px}
    .strat-t{font-size:14px;line-height:1.85;color:var(--text)}
    .psec{margin-bottom:16px}
    .phdr{display:flex;align-items:center;gap:9px;margin-bottom:10px;padding-bottom:8px;border-bottom:1.5px solid #f0ebe0}
    .pbdg{padding:3px 12px;border-radius:20px;font-size:11px;font-weight:800}
    .pb-필수{background:#c0392b;color:#fff}
    .pb-추천{background:var(--gold);color:#fff}
    .pb-선택{background:#f0ebe0;color:#7a6228}
    .pdsc{font-size:11px;color:#999}
    .pcnt{margin-left:auto;font-size:11px;color:#bbb}
    .sc-필수{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}
    .sc-추천{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}
    .sc-선택{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}
    .scard{background:#fafaf8;border:1px solid #ede8de;border-radius:12px;padding:14px;transition:transform .2s,box-shadow .2s}
    ...
    .sname{font-size:15px;font-weight:800;color:var(--text);letter-spacing:-.3px}
    .cat{display:inline-block;padding:2px 8px;border-radius:20px;font-size:10px;font-weight:700}
    ...
    .sreason{font-size:12px;line-height:1.7;color:#6b6460;padding-bottom:9px;margin-bottom:9px;border-bottom:1px solid #ede8de}
    .ext-l{font-size:9px;font-weight:700;color:#8a6e1a;letter-spacing:.09em;text-transform:uppercase;margin-bottom:5px}
    .ext-list{list-style:none;display:flex;flex-direction:column;gap:4px}
    .ext-item{display:flex;align-items:flex-start;gap:6px;font-size:11px;color:#6b6460;line-height:1.5}
    .ext-dot{width:3px;height:3px;background:var(--gold);border-radius:50%;margin-top:5px;flex-shrink:0}
    .rp-footer{margin-top:16px;text-align:center;font-size:11px;color:#bbb;line-height:1.7}
    ...
    @media(max-width:640px){
      ...
      .rp-wrap{padding:0 12px 44px;margin-top:-40px}
      ...
      .sc-필수,.sc-추천,.sc-선택{grid-template-columns:1fr;gap:8px}
      .scard{padding:12px}
      .sname{font-size:14px}
      .sreason{font-size:12px}
      .rp-home{top:14px;left:14px;font-size:12px}
      .strat-t{font-size:13px}
      .chips{gap:4px}
    ...
    @media print{
      .fp-back,.fp-logo,.rp-home,.rp-btns,.rp-footer,.toast,.tut-bar{display:none!important}
      body{background:#fff}
      .rp,.rp-wrap{background:#fff;padding:0}
      .rp-card{box-shadow:none;border:1px solid #ddd;border-radius:0}
...
const saveResult = async (form, results) => {
  const id = Math.random().toString(36).substr(2,9);
  const key = `curricula_${id}`;
  try {
    await fetch(`${SB_URL}/rest/v1/piltop_data`, {
      method:'POST',
      headers:{'apikey':SB_KEY,'Authorization':`Bearer ${SB_KEY}`,'Content-Type':'application/json','Prefer':'return=minimal'},
      body:JSON.stringify({key, value:`${form.career} ${form.major}`, jsonb:{form,results}})
    });
    return id;
  } catch { return null; }
};
...
const loadResult = async (id) => {
  try {
    const res = await fetch(`${SB_URL}/rest/v1/piltop_data?key=eq.curricula_${id}&select=jsonb`, {
      headers:{'apikey':SB_KEY,'Authorization':`Bearer ${SB_KEY}`}
    });
    const data = await res.json();
    return data?.[0]?.jsonb || null;
  } catch { return null; }
};

const SYSTEM = `당신은 2022 개정 교육과정(고교학점제) 전문 과목추천 AI입니다.
희망 대학이 있으면 웹 검색으로 해당 대학 최신 권장과목을 확인하세요.
순수 JSON만 반환. 마크다운 없이.

{"overall_strategy":"전략 2~3문장","subjects":[{"name":"과목명","category":"일반선택|진로선택|융합선택","priority":"필수|추천|선택","reason":"이유 2문장","extracurricular":["세특주제1","세특주제2"]}]}

priority: 필수=대학공식권장·전공직결 / 추천=진로강력연계 / 선택=보완심화
8~12개 (진로·전공 복잡도에 따라 유연하게): 필수2~4, 추천3~5, 선택1~3. 카테고리 고루.
고1→고2~3 전체로드맵 / 고2→고3집중.

[전공별 필수과목 규칙 — 반드시 필수에 포함]
의학·치의학·한의학: 화학(일반선택), 생명과학(일반선택), 미적분Ⅱ(진로선택)
약학·간호: 화학(일반선택), 생명과학(일반선택), 확률과 통계 또는 미적분Ⅱ
수의학: 화학(일반선택), 생명과학(일반선택)
생명과학·생물학·생명공학: 생명과학(일반선택), 화학(일반선택), 세포와 물질대사(진로선택)
화학·화학공학: 화학(일반선택), 물질과 에너지(진로선택), 화학 반응의 세계(진로선택)
물리학·물리치료: 물리학(일반선택), 역학과 에너지(진로선택), 미적분Ⅱ(진로선택)
기계·항공우주·로봇공학: 물리학(일반선택), 미적분Ⅱ(진로선택), 역학과 에너지(진로선택), 지구과학(일반선택)
전기·전자·반도체: 물리학(일반선택), 역학과 에너지 또는 전자기와 빛(진로선택), 미적분Ⅱ(진로선택)
컴퓨터·소프트웨어·AI: 정보(일반선택), 인공지능 기초(진로선택), 미적분Ⅱ 또는 확률과 통계
수학·통계·데이터과학: 미적분Ⅱ(진로선택), 기하(진로선택), 확률과 통계(일반선택)
건축·도시공학: 물리학(일반선택), 미적분Ⅱ(진로선택), 기하(진로선택)
지구과학·환경·기상: 지구과학(일반선택), 지구시스템과학(진로선택), 행성우주과학(진로선택)
경제·경영·경영학: 경제(진로선택), 미적분Ⅰ 또는 확률과 통계(일반선택), 사회와 문화(일반선택)
법학·행정·정치외교: 법과 사회(진로선택), 정치(진로선택), 현대사회와 윤리(일반선택)
심리학·교육학: 인간과 심리(교양진로), 사회와 문화(일반선택), 교육의 이해(교양진로)
국어국문·문예창작: 문학(일반선택), 독서와 작문(일반선택), 주제 탐구 독서(진로선택)
영어영문·국제학: 영어Ⅱ(일반선택), 심화 영어(진로선택), 영어 발표와 토론(진로선택)
역사학·고고학: 세계사(일반선택), 동아시아사 주제 탐구(진로선택), 역사로 탐구하는 현대 세계(융합선택)
사회복지·아동학: 사회와 문화(일반선택), 아동발달과 부모(기술가정진로), 인간과 심리(교양진로)
디자인·미술: 미술(일반선택), 미술 창작(진로선택), 미술 감상과 비평(진로선택)
음악: 음악(일반선택), 음악 연주와 창작(진로선택), 음악 감상과 비평(진로선택)
체육·스포츠과학: 체육1(일반선택), 운동과 건강(진로선택), 스포츠 과학(융합선택)

위 규칙에 해당하는 전공은 반드시 명시된 과목을 필수에 포함. 규칙에 없는 전공은 전공 특성에 맞게 추론.'

과목(아래 목록 외 절대 사용 금지):
국어일반:화법과 언어,독서와 작문,문학|국어진로:주제 탐구 독서,문학과 영상,직무 의사소통|국어융합:독서 토론과 글쓰기,매체 의사소통,언어생활 탐구
수학일반:대수,미적분Ⅰ,확률과 통계|수학진로:미적분Ⅱ,기하,경제 수학,인공지능 수학,직업 수학|수학융합:수학과 문화,실용 통계,수학과제 탐구
영어일반:영어Ⅰ,영어Ⅱ,영어 독해와 작문|영어진로:심화 영어,영미 문학 읽기,영어 발표와 토론,직무 영어|영어융합:실생활 영어 회화,미디어 영어,세계 문화와 영어
사회일반:세계시민과 지리,세계사,사회와 문화,현대사회와 윤리|사회진로:한국지리 탐구,도시의 미래 탐구,동아시아사 주제 탐구,정치,경제,법과 사회,윤리와 사상,인문학과 윤리,국제 관계의 이해|사회융합:여행지리,역사로 탐구하는 현대 세계,사회문제 탐구,금융과 경제생활,윤리문제 탐구,기후 변화와 지속가능한 세계
과학일반:물리학,화학,생명과학,지구과학|과학진로:역학과 에너지,전자기와 빛,물질과 에너지,화학 반응의 세계,세포와 물질대사,생물의 유전,지구시스템과학,행성우주과학|과학융합:과학의 역사와 문화,기후 변화와 환경생태,융합과학 탐구
정보일반:정보|정보진로:인공지능 기초,데이터 과학,소프트웨어와 생활|정보융합:정보 통신과 미래 사회
기술가정일반:기술·가정|기술가정진로:로봇과 공학세계,공학 일반,창의 공학 설계,지식 재산 일반,발명과 기업가 정신,인공지능과 미래 생활,아동발달과 부모,생애 설계와 자립|기술가정융합:가정과학,지식 재산과 경영,미래 기술과 사회
예술일반:음악,미술,연극|예술진로:음악 연주와 창작,음악 감상과 비평,미술 창작,미술 감상과 비평|예술융합:음악과 미디어,미술과 매체
체육일반:체육1,체육2|체육진로:운동과 건강,스포츠 생활1,스포츠 생활2|체육융합:스포츠 문화,스포츠 과학
교양진로:진로와 직업,생태와 환경,인간과 철학,삶과 종교,논리와 사고,인간과 심리,교육의 이해,보건|교양융합:인간과 경제활동,논술`;

const LSTEPS=['진로·전공 분석','대학 권장과목 웹 검색','2022 개정 과목 매칭','세특 탐구 주제 생성'];
...
function printPage(){
  ...
  if(/iPad|iPhone|iPod/.test(ua)){window.print();}
...
function FormPage({onBack,onResult}){
  const [form,setForm]=useState({grade:'1',career:'',major:'',university:''});
  const [error,setError]=useState(null);
  const [loading,setLoading]=useState(false);
  const [lstep,setLstep]=useState(0);
  const set=k=>e=>setForm(p=>({...p,[k]:e.target.value}));
  const valid=form.career.trim()&&form.major.trim();

  const submit=async()=>{
    setLoading(true);setError(null);setLstep(0);
    const t=setInterval(()=>setLstep(s=>s<LSTEPS.length-1?s+1:s),3500);
    const msg=`고${form.grade}학년, 희망진로:${form.career}, 희망전공:${form.major}${form.university?`, 희망대학:${form.university}`:''}`;
    try{
      const res=await fetch('/api/proxy',{method:'POST',headers:{'Content-Type':'application/json'},
        body:JSON.stringify({model:'claude-sonnet-4-6',max_tokens:2000,system:SYSTEM,
      ...
      if(!jm) throw new Error('결과 파싱 오류. 다시 시도해주세요.');
      clearInterval(t);
      const results=JSON.parse(jm[0]);
      // Supabase에 저장
      const id=await saveResult(form,results);
      onResult({results,form,shareId:id});
    }catch(e){clearInterval(t);setError(`오류: ${String(e.message)}`);setLoading(false);}
  };
  ...
  return(
    <div className="fp">
      <div className="fp-top">
        ...
        <p className="fp-sub">진로 맞춤 선택과목 AI 설계</p>
      </div>
      <div className="fp-wrap">
        <div className="fp-card">
          <div className="fp-ctitle">학년 · 진로 · 전공을 입력해주세요</div>
          <div className="fp-row">
            <div className="fp-grp"><label className="fp-lbl">학년<span className="req">*</span></label><select className="fp-ctrl" value={form.grade} onChange={set('grade')} style={{fontSize:16}}><option value="1">고1</option><option value="2">고2</option></select></div>
            <div className="fp-grp"><label className="fp-lbl">희망 진로<span className="req">*</span></label><input className="fp-ctrl" placeholder="예: 의사, 소프트웨어 엔지니어…" value={form.career} onChange={set('career')}/></div>
      ...
      <div className={`sc-${priority}`}>
        {list.map((s,i)=>(
          <div key={i} className="scard">
            <div className="sc-top"><span className="sname">{s.name}</span><span className={catCls(s.category)}>{s.category}</span></div>
...
function ResultsPage({results,form,shareId,onBack,isShared}){
  ...
  const shareUrl=shareId
    ? `${location.origin}${location.pathname}?r=${shareId}`
    : null;

  const copy=()=>{
    if(!shareUrl){setToast('링크 생성 중 오류가 발생했습니다.');setTimeout(()=>setToast(''),2500);return;}
    navigator.clipboard.writeText(shareUrl).then(()=>{
      setToast('✓ 링크가 복사되었습니다');setTimeout(()=>setToast(''),2500);
    }).catch(()=>{
      // 모바일 fallback
      const ta=document.createElement('textarea');
      ta.value=shareUrl;document.body.appendChild(ta);ta.select();
      try{document.execCommand('copy');setToast('✓ 링크가 복사되었습니다');}
      catch{setToast('주소창에서 직접 복사해주세요');}
  ...
  return(
    <div className="rp">
      <div className="rp-top">
        {!isShared&&<button className="rp-home" onClick={onBack}>← 다시 입력</button>}
      ...
      <div className="rp-wrap">
        <div className="rp-card">
          <div className="rp-top2">
            <div className="chips">
              <span className="chip">고{form.grade}</span>
              <span className="chip">{form.career}</span>
              <span className="chip">{form.major}</span>
              {form.university&&<span className="chip">{form.university}</span>}
            </div>
            <div className="rp-btns">
              {shareUrl&&<button className="rb-lnk" onClick={copy}>🔗 링크 복사</button>}
...
function App(){
    ...
    try{
      ...
      if(rid){
        ...
        loadResult(rid).then(d=>{
          if(d?.results&&d?.form){
            setRdata({...d,shareId:rid,isShared:true});
            setPage('results');
          }
          setLoading(false);
  ...
  const goForm=()=>{setPage('form');window.scrollTo(0,0);};
  const goHome=()=>{setPage('landing');window.scrollTo(0,0);history.pushState('','',location.pathname);};
  const goResult=d=>{setRdata(d);setPage('results');
    if(d.shareId) history.pushState('','',`${location.pathname}?r=${d.shareId}`);
    window.scrollTo(0,0);
  };

  if(page==='landing') return <Landing onStart={goForm}/>;
  if(page==='form') return <FormPage onBack={goHome} onResult={goResult}/>;
  if(page==='results'&&rdata) return <ResultsPage results={rdata.results} form={rdata.form} shareId={rdata.shareId} onBack={()=>{setPage('form');history.pushState('','',location.pathname);}} isShared={rdata.isShared}/>;
  return null;
}
